...
bearing.regs.auto <- lapply(dflist.blocks, FUN = function(x) 
  return(lm(bearing.change ~ lat + long + bearing.prev + speed.prev, # no lag
            data = x)))

speed.regs.auto <- lapply(dflist.blocks, FUN = function(x) 
  return(lm(speed.change ~ lat + long + bearing.prev + speed.prev,  # no lag
            data = x)))
...
# Store starting points - need 2 rows to get lag on bearing.change/speed.change
# and speed.change. (Second row is first row where change is recorded, then 2 lags.)
...
names <- temp
loc <- "data/generate/Validation_Sims_No_Auto_plus/"
...
#> proc.time() - ptm
#    user   system  elapsed 
#2343.157   12.567 2373.512

for(i in 1:length(names)){
  
  newdir <- paste0(loc, substr(names[i],1,nchar(names[i])-4),"/")
  ### to make new folders with bash (when in the "loc" location:
