...
#' 
error_bands_bubbleCI_NSWE<- function(bubble_ci_list, long = 1, lat = 2, distance = 3, 
...
#' 
#' @description
#' This function takes in a list of dataframes, representing the different CI bubbles
#' built on the track of the most "central" points, and extract center points, radiuses and
#' extreme of the CI bubbles per each step
#' 
#' @param bubble_steps_CI list of dataframes, with each dataframe representing one step in 
#' the track of the most central TC
...
error_bands_bubbleCI <- function(bubble_steps_CI, long_col = 1, lat_col = 2, 
  ...
  centers <- data.frame(long = as.numeric(centers[, long_col]),
                        lat = as.numeric(centers[, lat_col])) # dataframe in list column problem
  ...
  for (i in 1:nrow(centers)) {
    radius_ind <- uconv(radius[i], from = unit_measure, to = "m", "Length")
...
max_cumulative_area <- function(tc_bubble_structure) {
  ...
  if (!(is.data.frame(tc_bubble_structure$negative)) & !(is.matrix(tc_bubble_structure$negative))){
  ...
  for (i in 2:nrow(neg)) {
    dat_all <- rbind(pos[1:i,],
                     neg_back[(n - i):n,])
    spPoly <- SpatialPolygons(list(Polygons(list(Polygon(dat_all)),ID = 1)))
    area_vec <- c(area_vec, gArea(spPoly))
  }  
...
bubble_ci_from_tclist <- function(dflist, center_idx, alpha_level = 0.1, 
    ...
    bubble_steps_CI <- bubbleCI(dflist = dflist, center_idx = center_idx, alpha_level = alpha_level, 
                                long = long, lat = lat, output_length = unit_measure)

    bubble_ci_structure <- error_bands_bubbleCI(bubble_steps_CI = bubble_steps_CI, 
                                                long_col = long, lat_col = lat, unit_measure = unit_measure)
