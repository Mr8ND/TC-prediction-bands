...
compression_points = function(df2,lonlat = TRUE,output_length="nautical mile"){
  ...
  # ^ probably should be 13 x 3 (lon, lat, speed)
  ...
  speed[length(speed)] = speed[length(speed)-1] # correct for NA in the last position

  new_12speed       = data.frame(matrix(0,nrow = 12,ncol = 1))    #### new
  ...
  return(list(new_13compression = new_13compression,new_12speed = new_12speed))
}

compression_points_listable = function(list_df,c_position = 5:6,lonlat =TRUE){
