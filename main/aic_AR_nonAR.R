...
# It plots geographical maps with percent decrease in AIC.
...
for(i in 1:length(aic_chg_bearing)){
  ...
  aic_AR <- extractAIC(bearing_regs_auto[[block_name]])[2]
  aic_nonAR <- extractAIC(bearing_regs_nonauto[[block_name]])[2]
  
  aic_chg_bearing[[block_name]] <- (aic_AR - aic_nonAR) / aic_nonAR
}
...
for(i in 1:length(aic_chg_speed)){
  ...
  aic_AR <- extractAIC(speed_regs_auto[[block_name]])[2]
  aic_nonAR <- extractAIC(speed_regs_nonauto[[block_name]])[2]
  
  aic_chg_speed[[block_name]] <- (aic_AR - aic_nonAR) / aic_nonAR
}
...
west_bounds$aic_chg_bearing[west_bounds$aic_chg_bearing > 0] <- NA
west_bounds$aic_chg_speed[west_bounds$aic_chg_speed > 0] <- NA
...
east_bounds$aic_chg_bearing[east_bounds$aic_chg_bearing > 0] <- NA
east_bounds$aic_chg_speed[east_bounds$aic_chg_speed > 0] <- NA
...
bearing_map <- ggplot(all_bounds) +
  labs(x = "Longitude", y = "Latitude", fill = "% Chg\nin AIC",
  ...
  geom_rect(data = all_bounds, 
            aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, 
                fill = aic_chg_bearing), 
            colour = "black", alpha = 0.8,
            size = 0.35, inherit.aes = T) +
  facet_grid(. ~ direction_bearing) +
  scale_fill_gradient(na.value = "white", low = "red", high = "pink",
                      limits = c(min(min(west_bounds$aic_chg_bearing, na.rm = T), 
                                     min(east_bounds$aic_chg_bearing, na.rm = T)),
                                 0),
                      labels = percent)
...
speed_map <- ggplot(all_bounds) +
  ...
  scale_fill_gradient(na.value = "white", low = "red", high = "pink",
                      limits = c(min(min(west_bounds$aic_chg_speed, na.rm = T), 
                                     min(east_bounds$aic_chg_speed, na.rm = T)),
                                 0),
                      labels = percent)
