...
### b <- load("main/data/sim_validation_results_summary_data2018-07-12.Rdata")
...
df_area <- df_area %>%
    mutate(sim = factor(sim,
                             ...
                             labels = c("Auto Regression, Kernel Death",
                                        ...
                                        "Non Auto Regression, Bernoulli Death")),
           cb = factor(cb,
                            levels = c("bubble_ci", "kde",
                                       "delta_ball",  "convex_hull"),
                            labels = c("Pointwise Bubble Estimate",
                                       "Kernel Density Estimate",
                                       "Delta Ball Covering",
                                       "Convex Hull")))
...
df_area %>% ggplot() +
  geom_density(aes(x = area, color = cb)) +
  facet_wrap(~sim, labeller = label_wrap_gen(width = 25)) +
  theme_minimal() +
  theme(strip.background = element_rect(fill = "grey90", color = NA)) +
  scale_color_manual(values = c("#d7191c", "#fdae61",
                                "#abdda4", "#2b83ba")) +
...
df_time <- data.frame(depth = -1, data_deep = -1, dist = -1, kde = -1,
                      bubble = -1, delta = -1, convex = -1,
                      tc = "ben", sim = "ben") %>%
  ...
  for (sim in names(output_list_pipeline[[tc]])) {
    ...
    df_time <- rbind(df_time,
                     data.frame(
                      ...
                      dist = time_out$dist,
                      kde = time_out$kde,
                      bubble = time_out$bubble,
                      delta = time_out$delta,
                      convex = time_out$convex,
                      tc = tc, sim = sim))
  }
}
...
df_time <- df_time %>% mutate(full_kde = kde,
                              full_bubble = dist + depth + bubble,
                              full_delta = dist + depth + data_deep + delta,
                              full_convex = dist + depth + data_deep + convex)

df_time2 <- df_time %>%
  reshape2::melt(id.vars = c("tc", "sim"),
                 measure.vars = c("full_kde", "full_bubble",
                                  "full_delta", "full_convex"))

df_time2 %>% ggplot() +
  geom_histogram(aes(x = value)) +
  facet_grid(sim ~ variable)


df_time3 <- df_time2 %>% group_by(sim, variable) %>%
  summarize(time = paste0(round(mean(value),2),
                          " $pm$ ", round(sd(value),2))) %>%
            reshape2::dcast(sim ~ variable)


df_time4 <- df_time3 %>% as.matrix()

colnames(df_time4) <- c(".", "KDE", "Spherical",
                        "Delta Ball", "Convex Hull")
rownames(df_time4) <- c("Auto Regression, Binomial Death",
                        "Auto Regression, Kernel Death",
                        "Non Auto Regression, Binomial Death",
                        "Non Auto Regression, Kernel Death")
...
df_time_p <- data.frame(full_kde = -1,
                        full_bubble = -1,
                        full_delta = -1,
                        full_convex = -1,
                        tc = "ben", sim = "ben") %>%
...
for (tc in 1:length(simulation_validation_pipeline100)) {
  for (sim in names(simulation_validation_pipeline100[[tc]])) {
    ...
    df_time_p <- rbind(df_time_p,
                     data.frame(
                      full_kde = time_out["kde"],
                      full_bubble = time_out["bubble"],
                      full_delta = time_out["delta"],
                      full_convex = time_out["convex"],
                      tc = tc, sim = sim))
  }
}
...
df_time_p3 <- df_time_p2 %>% group_by(sim, variable) %>%
  summarize(time = paste0(round(mean(value),2),
                          " pm ", round(sd(value),2))) %>%
            reshape2::dcast(sim ~ variable)


df_time_p4 <- df_time_p3 %>% as.matrix()

colnames(df_time_p4) <- c(".", "KDE", "Spherical",
                        "Delta Ball", "Convex Hull")
rownames(df_time_p4) <- c("Auto Regression, Binomial Death",
                        "Auto Regression, Kernel Death",
                        "Non Auto Regression, Binomial Death",
                        "Non Auto Regression, Kernel Death")
